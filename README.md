...
3. **直感的なリッチテキスト・ブロックエディタ (BlockNote)**
   詳細メモはNotionのようにブロック単位で直感的に編集可能。 `#` を入力して見出しに変換するなど、Markdownライクなショートカットにリアルタイム対応しています。データ自体はMarkdownテキストとして安全かつポータブルに保存されます。
4. **爆速のリアルタイム検索**
   タイトル、詳細メモ、カテゴリ、タグから部分一致で瞬間的にエントリを検索可能です。
5. **エントリ間の双方向リンク（社内Wiki化）**
   エディタ内で `@` を入力すると人物などへのメンションリンクが、 `[[` を入力すると別エントリへのWikiリンクがサジェストされます。「閲覧モード」ではクリック一つで瞬時に対象のエントリへジャンプでき、知識がネットワーク状に繋がります。「編集モード」に切り替えることで、安全にテキストのみを編集することが可能です。
6. **柔軟なカテゴリ・アイコン管理**
   デフォルトのカテゴリに加えて、ユーザー自身でカテゴリを新規作成・編集・削除できます。Lucideの各種プリセットアイコンからお気に入りのものを設定可能です。
7. **ローカルバックアップ (AES暗号化) ＆ 復元**
...
8. **アプリ内説明書（ヘルプモーダル）**
   サイドバーからいつでもアプリの特徴や使い方ガイドを参照可能です。
9. **徹底した軽量化・最適化処理**
   Reactの `useMemo` フックやオンデマンドのDBクエリ取得により、検索やサジェスト表示時の無駄な再レンダリングを限りなく抑え、ブラウザ上でも大規模なテキストデータをサクサク扱うことができます。
...
- **BlockNote (@blocknote/core, @blocknote/react)**: Notionライクなリッチテキスト入力・表示を可能にするブロックベースのWYSIWYGエディタ。内部でMarkdown形式への変換・保存を担当します。
...
- **内部リンク（Wikiリンク・メンション）**: 詳細メモの編集中に `@` または `[` を入力すると登録済みエントリのサジェストが表示され、選択するとリンクとして挿入されます。「閲覧モード」で青いリンクテキストをクリックすると、そのエントリへ一瞬でジャンプします。「編集モード」に切り替えると、誤クリックを防止しつつ安全にテキストの修正が可能です。
...
- **使い方 / ヘルプ**: サイドバーの下部（設定ボタンのすぐ上）にある「使い方 / ヘルプ」ボタンをクリックすると、アプリの特徴や基本的な使い方が確認できるモーダルが開きます。初めて触る場合は、まずこちらをご覧ください。
